(function () {
  ...
  document.querySelectorAll('[data-landing-utm]').forEach(function (form) {
  ...
  function polishLayout() {
    ...
    main.querySelectorAll('aside.search-aside').forEach(function (aside) {
      ...
      if (aside.closest('.me-page') || aside.querySelector('[data-me-filters-toggle]')) return;
  ...
  function readSuggestCatalogs() {
    var node = document.getElementById('vitrine-suggests');
    if (!node || node.dataset.ready) {
      return node && node._catalogs ? node._catalogs : {};
    }
    try {
      node._catalogs = JSON.parse(node.textContent || '{}');
    } catch (err) {
      node._catalogs = {};
    }
    node.dataset.ready = '1';
    return node._catalogs;
  }

  function currentSuggestToken(input, split) {
    var raw = input.value || '';
    if (!split) return raw.trim();
    var caret = input.selectionStart != null ? input.selectionStart : raw.length;
    var left = raw.slice(0, caret);
    var match = left.match(/[^,;]*$/);
    return (match ? match[0] : '').replace(/^\s+/, '');
  }

  function applySuggestValue(input, value, split) {
    if (!split) {
      input.value = value;
      return;
    }
    var raw = input.value || '';
    var caret = input.selectionStart != null ? input.selectionStart : raw.length;
    var left = raw.slice(0, caret).replace(/[^,;]*$/, '');
    var right = raw.slice(caret).replace(/^[^,;]*/, '');
  ...
  function takenSuggestValues(input, split) {
    var taken = {};
    if (split) {
      var token = foldText(currentSuggestToken(input, split));
      String(input.value || '').split(/[,;]+/).forEach(function (part) {
        var key = foldText(part);
        if (key && key !== token) taken[key] = true;
      });
      return taken;
    }
    var catalog = input.getAttribute('data-suggest');
    var form = input.form || document;
    var selector = catalog === 'languages'
      ? '[data-suggest="languages"]'
      : '[data-suggest="' + catalog + '"]';
    form.querySelectorAll(selector).forEach(function (other) {
      if (other === input) return;
      var key = foldText(other.value);
      if (key) taken[key] = true;
    });
    return taken;
  }

  function filterSuggestItems(items, query, taken) {
    ...
    items.forEach(function (item) {
      ...
      if (!label) return;
      var folded = foldText(label);
      if (taken[folded]) return;
      if (!needle) {
        starts.push(label);
        return;
      }
      ...
      if (at === -1) return;
      if (at === 0) starts.push(label);
      else contains.push(label);
    });
    return starts.concat(contains).slice(0, 8);
  }

  function bindFieldSuggest(input) {
    if (!input || input.dataset.suggestBound) return;
    input.dataset.suggestBound = '1';
    var catalog = input.getAttribute('data-suggest');
    var split = input.getAttribute('data-suggest-split');
    var wrap = input.closest('.field-suggest') || input.parentElement;
    if (!wrap) return;
    wrap.classList.add('field-suggest');
    var panel = wrap.querySelector('[data-suggest-panel]');
    if (!panel) {
      panel = document.createElement('div');
      panel.className = 'search-suggest field-suggest-panel';
      panel.hidden = true;
      panel.setAttribute('role', 'listbox');
      panel.setAttribute('data-suggest-panel', '');
    ...
    function highlight(next) {
      var buttons = panel.querySelectorAll('[data-suggest-pick]');
    ...
    function choose(value) {
      applySuggestValue(input, value, split);
      input.focus();
      if (split) render();
      else close();
    }
    ...
    function render() {
      var catalogs = readSuggestCatalogs();
      var source = catalogs[catalog] || [];
      items = filterSuggestItems(source, currentSuggestToken(input, split), takenSuggestValues(input, split));
      if (!items.length) {
        close();
        return;
      }
      panel.innerHTML = items.map(function (label) {
        return '<button type="button" role="option" aria-selected="false" class="city-ac-item" data-suggest-pick><strong>' +
          escapeHtml(label) + '</strong></button>';
      }).join('');
      panel.querySelectorAll('[data-suggest-pick]').forEach(function (btn, i) {
  ...
  function bindFieldSuggests(root) {
    var scope = root || document;
    scope.querySelectorAll('[data-suggest]').forEach(bindFieldSuggest);
    if (root && root.matches && root.matches('[data-suggest]')) bindFieldSuggest(root);
  }

  bindFieldSuggests(document);
  ...
  document.addEventListener('click', function (event) {
    var tabGoto = event.target.closest('[data-tab-goto]');
    if (tabGoto && document.querySelector('.vitrine-page')) {
      event.preventDefault();
      showVitrineTab(tabGoto.getAttribute('data-tab-goto') || 'identite');
      return;
    }

    var copyFrom = event.target.closest('[data-copy-from]');
    if (copyFrom) {
      event.preventDefault();
      var source = document.querySelector(copyFrom.getAttribute('data-copy-from') || '');
      var fromValue = source ? source.value : '';
      if (!fromValue) return;
      copyText(fromValue).then(function () {
    ...
    var copyLanding = event.target.closest('[data-copy-landing]');
    if (copyLanding) {
      event.preventDefault();
      var base = copyLanding.getAttribute('data-url') || '';
      if (!base) return;
      var form = document.querySelector('[data-landing-utm]');
      var params = [];
      var readUtm = function (name, fallback) {
        var field = form ? form.querySelector('[name="' + name + '"]') : null;
        var raw = field && field.value ? String(field.value).trim() : '';
        if (!raw && fallback) raw = fallback;
        return raw;
      };
      var source = readUtm('utm_source', 'google');
      var medium = readUtm('utm_medium', 'cpc');
      var campaign = readUtm('utm_campaign', copyLanding.getAttribute('data-campaign') || '');
      var content = readUtm('utm_content', '');
      if (source) params.push('utm_source=' + encodeURIComponent(source));
      if (medium) params.push('utm_medium=' + encodeURIComponent(medium));
      if (campaign) params.push('utm_campaign=' + encodeURIComponent(campaign));
      if (content) params.push('utm_content=' + encodeURIComponent(content));
      var landingUrl = base + (params.length ? (base.indexOf('?') >= 0 ? '&' : '?') + params.join('&') : '');
      copyText(landingUrl).then(function () {
        showToast('URL campagne copiée.');
      }).catch(function () {
        showToast('Impossible de copier.');
      });
      return;
  ...
  function bindFilePick(pick) {
    ...
    input.addEventListener('change', function () {
      ...
      var tooBig = maxBytes > 0 && files.some(function (file) { return file.size > maxBytes; });
      var error = pick.parentElement && pick.parentElement.querySelector('[data-portfolio-file-error], [data-file-size-error]');
      if (tooBig) {
        input.value = '';
        setFiles(null);
        if (error) {
          error.textContent = 'Ce fichier dépasse 10 Mo.';
          error.hidden = false;
        }
        return;
      }
  ...
  var PORTFOLIO_HELP = {
    image: 'JPG, PNG, WebP ou GIF — 10 Mo max.',
    pdf: 'PDF — 10 Mo max.',
    audio: 'MP3, WAV, M4A, OGG ou AAC — 10 Mo max.',
    text: 'Quelques lignes ou un passage. Pas de fichier, 6 000 caractères max.'
  };
  var PORTFOLIO_ACCEPT = {
    image: 'image/jpeg,image/png,image/webp,image/gif',
    pdf: 'application/pdf,.pdf',
    audio: 'audio/mpeg,audio/wav,audio/mp4,audio/ogg,audio/aac,.mp3,.wav,.m4a,.ogg,.aac',
    text: ''
  };
  var PORTFOLIO_FILE_LABEL = {
    image: 'Visuel',
    pdf: 'Fichier PDF',
    audio: 'Fichier sonore'
  };

  function isUnsupportedPortfolioFile(file, media) {
    var name = (file.name || '').toLowerCase();
    var type = (file.type || '').toLowerCase();
    if (media === 'pdf') {
    ...
    if (media === 'audio') {
      return !/\.(mp3|wav|m4a|ogg|aac)$/.test(name) && type.indexOf('audio/') !== 0;
    }
  ...
  function bindPortfolioPreview(row) {
    ...
    var mediaSelect = row.querySelector('[data-portfolio-media]');
    var fileInput = row.querySelector('input[type="file"]');
    var urlInput = row.querySelector('input[name*="[image_url]"]');
    var preview = row.querySelector('[data-portfolio-preview]');
    var filename = row.querySelector('[data-portfolio-filename]');
    var error = row.querySelector('[data-portfolio-file-error]');
    var help = row.querySelector('[data-portfolio-help]');
    var fileLabel = row.querySelector('[data-portfolio-file-label]');
    ...
    function syncMedia() {
      ...
      row.setAttribute('data-media-type', media);
      row.querySelectorAll('[data-portfolio-panel]').forEach(function (panel) {
        var key = panel.getAttribute('data-portfolio-panel');
        if (key === 'text') panel.hidden = media !== 'text';
        else if (key === 'file') panel.hidden = media === 'text';
        else if (key === 'image-url') panel.hidden = media !== 'image';
      });
      if (help) help.textContent = PORTFOLIO_HELP[media] || PORTFOLIO_HELP.image;
      if (fileLabel) fileLabel.textContent = PORTFOLIO_FILE_LABEL[media] || 'Fichier';
      if (fileInput) {
        fileInput.setAttribute('accept', PORTFOLIO_ACCEPT[media] || '');
        fileInput.value = '';
      }
    ...
    if (mediaSelect) {
      mediaSelect.addEventListener('change', syncMedia);
      mediaSelect.addEventListener('input', syncMedia);
    }
    ...
    if (fileInput) {
      fileInput.addEventListener('change', function () {
        ...
        var maxBytes = parseInt(fileInput.getAttribute('data-max-bytes') || '10485760', 10);
        if (file.size > maxBytes) {
          setError('Ce fichier dépasse 10 Mo.');
          fileInput.value = '';
          setPreview('');
          setFilename('');
          return;
        }
        if (isUnsupportedPortfolioFile(file, media)) {
          setError(media === 'pdf'
            ? 'Envoyez un PDF.'
            : (media === 'audio' ? 'Envoyez un MP3, WAV, M4A, OGG ou AAC.' : 'Envoyez un JPG, PNG, WebP ou GIF.'));
          fileInput.value = '';
          setPreview('');
          setFilename('');
          return;
  ...
  function initWysiwyg(wrap) {
    ...
    var bubble = wrap.querySelector('[data-wysiwyg-bubble]');
    ...
    [toolbar, bubble].forEach(function (controls) {
      if (!controls) return;
      ...
      controls.addEventListener('click', function (event) {
        ...
        if (!btn) return;
        runEditorCommand(btn);
      });
    });

    if (bubble && wrap.hasAttribute('data-floating-toolbar')) {
      function updateBubble() {
        ...
        var top = rect.top - wrapRect.top - bubble.offsetHeight - 10;
        bubble.style.left = Math.max(92, Math.min(wrapRect.width - 92, left)) + 'px';
        bubble.style.top = Math.max(4, top) + 'px';
      }
      editor.addEventListener('mouseup', updateBubble);
      editor.addEventListener('keyup', updateBubble);
      document.addEventListener('selectionchange', function () {
        window.requestAnimationFrame(updateBubble);
      });
      editor.addEventListener('blur', function () {
        window.setTimeout(function () {
          if (!wrap.contains(document.activeElement)) bubble.hidden = true;
        }, 0);
      });
    }
  }
  ...
  (function initTribuneStudio() {
    var form = document.querySelector('[data-tribune-form]');
    if (!form) return;
    var title = form.querySelector('[name="title"]');
    var image = form.querySelector('[data-tribune-image]');
    var removeImage = form.querySelector('[data-tribune-remove-image]');
    var coverPreview = form.querySelector('[data-tribune-cover-preview]');
    var state = form.querySelector('[data-save-state]');
    var stateLabel = state && state.querySelector('[data-save-label]');
    var saveTimer = 0;
    var saving = null;
    var revision = 0;
    var imageUrl = '';
    var initialImage = coverPreview ? coverPreview.style.backgroundImage : '';

    function setState(name, label) {
      if (state) state.setAttribute('data-state', name);
    ...
    function markDirty() {
      ...
      setState('dirty', 'Modifications non enregistrées');
      if (title && title.value.trim()) {
        saveTimer = window.setTimeout(function () { saveDraft(); }, 6000);
      }
    }
    ...
    function updateCoverPreview() {
      ...
      coverPreview.style.backgroundImage = bg;
      coverPreview.classList.toggle('is-empty', !bg);
    }

    function saveDraft() {
      if (saving) return saving;
      if (!title || !title.value.trim()) {
        setState('dirty', 'Ajoutez un titre pour activer la sauvegarde');
        return Promise.reject(new Error('Le titre est obligatoire.'));
      }
      window.clearTimeout(saveTimer);
      setState('saving', 'Enregistrement…');
      var savingRevision = revision;
      var data = new FormData(form);
      data.delete('action');
      saving = fetch(form.getAttribute('data-autosave-url'), {
        method: 'POST',
        body: data,
        ...
        return response.json().catch(function () { return {}; }).then(function (payload) {
          if (!response.ok || !payload.ok) throw new Error(payload.error || 'La sauvegarde a échoué.');
          return payload;
        });
      }).then(function (payload) {
        form.setAttribute('data-autosave-url', payload.autosave_url);
        form.setAttribute('data-preview-url', payload.preview_url);
        ...
        if (coverPreview) {
          ...
          initialImage = payload.cover_url ? 'url("' + String(payload.cover_url).replace(/"/g, '%22') + '")' : '';
          coverPreview.style.backgroundImage = initialImage;
          coverPreview.classList.toggle('is-empty', !initialImage);
        }
        if (image && image.files && image.files.length) {
          image.value = '';
          var pick = image.closest('[data-file-pick]');
          var name = pick && pick.querySelector('[data-file-name]');
          if (pick) pick.classList.remove('has-file');
          if (name) name.textContent = 'Image enregistrée';
        }
        ...
        if (revision === savingRevision) {
          setState('saved', 'Enregistré à ' + payload.saved_at);
        } else {
          setState('dirty', 'Nouvelles modifications à enregistrer');
          saveTimer = window.setTimeout(function () { saveDraft(); }, 1500);
        }
        return payload;
      }).catch(function (error) {
        setState('error', error.message || 'La sauvegarde a échoué.');
        throw error;
      }).finally(function () {
        saving = null;
      });
    ...
    if (image) image.addEventListener('change', updateCoverPreview);
    if (removeImage) removeImage.addEventListener('change', updateCoverPreview);
    ...
    document.querySelectorAll('[data-tribune-preview-open]').forEach(function (button) {
      button.addEventListener('click', function () {
        var tab = window.open('about:blank', '_blank');
        if (tab) tab.document.write('<title>Préparation de l’aperçu…</title><p style="font:16px sans-serif;padding:30px">Enregistrement et préparation de l’aperçu…</p>');
        saveDraft().then(function (payload) {
          if (tab) {
            tab.location.href = payload.preview_url;
          } else {
    ...
    window.addEventListener('beforeunload', function (event) {
      if (!state || ['dirty', 'saving'].indexOf(state.getAttribute('data-state')) === -1) return;
      event.preventDefault();
      event.returnValue = '';
    });
  })();
  ...
  function downloadShareSvg(root) {
    var svg = shareKitSvg(root);
    if (!svg) return;
    var name = (root.getAttribute('data-file') || 'visuel') + '.svg';
    downloadBlob(name, new Blob([serializeSvg(svg)], { type: 'image/svg+xml;charset=utf-8' }));
  }

  function downloadSharePng(root) {
    var svg = shareKitSvg(root);
    if (!svg) return;
    var width = parseInt(root.getAttribute('data-width') || '1080', 10);
    var height = parseInt(root.getAttribute('data-height') || '1080', 10);
    var name = (root.getAttribute('data-file') || 'visuel') + '.png';
    ...
    ready.then(function () { image.src = href; }).catch(function () { image.src = href; });
  }

  document.querySelectorAll('[data-share-visual]').forEach(function (root) {
    var pngBtn = root.querySelector('[data-share-png]');
    var svgBtn = root.querySelector('[data-share-svg]');
    if (pngBtn) {
      pngBtn.addEventListener('click', function () { downloadSharePng(root); });
    }
    if (svgBtn) {
