...
final class ForumTopic
{
    ...
    public static function findByArticle(int $articleId, bool $visibleOnly = true): ?array
    {
        $where = 't.article_id = ?';
        if ($visibleOnly) {
            $where .= ' AND t.status = "visible"';
        }
        $row = self::baseFetch($where, [$articleId]);
        return $row ? self::present($row) : null;
    }

    public static function findBySalon(int $salonId, bool $visibleOnly = true): ?array
    {
        $where = 't.salon_id = ?';
        if ($visibleOnly) {
            $where .= ' AND t.status = "visible"';
        }
        $row = self::baseFetch($where, [$salonId]);
        return $row ? self::present($row) : null;
    }
    ...
    public static function commentOnArticle(array $article, int $userId, array $data): array
    {
        ...
        $existing = self::findByArticle($articleId, false);
        if ($existing) {
            self::assertCommentable($existing);
            return [
                'topic' => $existing,
     ...
     * @return array{topic: array<string, mixed>, post: array<string, mixed>, created: bool}
     */
    public static function commentOnSalon(array $salon, int $userId, array $data): array
    {
        $salonId = (int) ($salon['id'] ?? 0);
        if ($salonId <= 0) {
            throw new RuntimeException('Ce salon n’est plus dans l’agenda.');
        }

        $existing = self::findBySalon($salonId, false);
        if ($existing) {
            self::assertCommentable($existing);
            return [
                'topic' => $existing,
        ...
        $category = self::categoryForSalon();
        $categoryId = (int) ($category['id'] ?? 0);
        if ($categoryId <= 0) {
            throw new RuntimeException('La rubrique du forum est introuvable.');
        }

        $title = trim((string) ($salon['name'] ?? ''));
        if ($title === '') {
            $title = 'Discussion autour d’un salon';
        }
        $title = mb_substr($title, 0, 180);
        $safeBody = sanitize_user_html($body);
        $tags = self::normalizeTags(['salon', (string) ($salon['kind'] ?? $salon['category'] ?? '')]);
        $slug = unique_slug(
            mb_substr('salon-' . (string) ($salon['slug'] ?? $title), 0, 180),
            static fn (string $candidate): bool => Database::fetch(
                'SELECT id FROM forum_topics WHERE category_id = ? AND slug = ?',
                [$categoryId, $candidate]
            ) !== null
        ...
        try {
            return Database::transaction(static function () use (
                ...
                Database::query(
                    'INSERT INTO forum_topics
                        (category_id, salon_id, user_id, title, slug, tags_json, reply_count, view_count,
                         follow_count, last_post_at, last_post_user_id, status, created_at, updated_at)
                     VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, NOW(), ?, "visible", NOW(), NOW())',
                    [$categoryId, $salonId, $userId, $title, $slug, self::encodeTags($tags), $userId]
                );
                $topicId = (int) Database::lastId();
                Database::query(
                    'INSERT INTO forum_posts
            ...
            // La contrainte UNIQUE(salon_id) arbitre deux premiers commentaires simultanés.
            $existing = self::findBySalon($salonId, false);
            if (!$existing) {
                throw $e;
            }
    ...
    /** @return array<string, mixed> */
    private static function categoryForSalon(): array
    {
        return ForumCategory::findBySlug('diffusion-et-librairies')
            ?? ForumCategory::findBySlug('divers')
            ?? [];
    }

    /** @param array<string, mixed> $topic */
    private static function assertCommentable(array $topic): void
    {
        if (($topic['status'] ?? '') !== 'visible' || !empty($topic['is_locked'])) {
            throw new RuntimeException('Cette discussion est actuellement fermée aux nouveaux commentaires.');
        }
    }
    ...
    public static function present(array $row): array
    {
        ...
        $topic = [
            ...
            'article_id' => isset($row['article_id']) && $row['article_id'] !== null ? (int) $row['article_id'] : null,
            'article_href' => !empty($row['article_slug']) ? '/journal/' . $row['article_slug'] : null,
            'salon_id' => isset($row['salon_id']) && $row['salon_id'] !== null ? (int) $row['salon_id'] : null,
            'salon_href' => !empty($row['salon_slug']) ? '/salons/' . $row['salon_slug'] : null,
