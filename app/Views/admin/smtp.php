...
<form method="post" action="<?= e(url('/admin/smtp')) ?>" style="max-width:640px;display:flex;flex-direction:column;gap:14px;background:#fff;padding:24px;border-radius:14px;">
  <?= csrf_field() ?>
  <?php $s = $settings ?? []; ?>
  <div><label class="field">Hôte</label><input class="input" name="mail_host" value="<?= e($s['mail_host'] ?? '') ?>"></div>
  <div><label class="field">Port</label><input class="input" name="mail_port" value="<?= e($s['mail_port'] ?? '587') ?>"></div>
  <div><label class="field">Utilisateur</label><input class="input" name="mail_username" value="<?= e($s['mail_username'] ?? '') ?>"></div>
  <div><label class="field">Mot de passe</label><input class="input" type="password" name="mail_password" value="<?= e($s['mail_password'] ?? '') ?>"></div>
  <div>
    <label class="field">Chiffrement</label>
    <select class="input" name="mail_encryption">
      <option value="tls" <?= (($s['mail_encryption'] ?? '') === 'tls') ? 'selected' : '' ?>>TLS</option>
      <option value="ssl" <?= (($s['mail_encryption'] ?? '') === 'ssl') ? 'selected' : '' ?>>SSL</option>
      <option value="" <?= (($s['mail_encryption'] ?? '') === '') ? 'selected' : '' ?>>Aucun</option>
    </select>
  </div>
  <div><label class="field">E-mail expéditeur</label><input class="input" name="mail_from_address" value="<?= e($s['mail_from_address'] ?? '') ?>"></div>
...
<form method="post" action="<?= e(url('/admin/smtp/test')) ?>" style="max-width:640px;margin-top:18px;display:flex;gap:10px;align-items:flex-end;">
  ...
  <div style="flex:1;">
    ...
    <input class="input" type="email" name="test_email" value="<?= e(auth_user()['email'] ?? '') ?>">
  </div>
  <button class="btn-navy" type="submit">Tester</button>
</form>
<p style="color:#8496A8;font-size:13px;max-width:640px;">Si aucun hôte SMTP n'est renseigné, les e-mails sont écrits dans <code>storage/mail</code>.</p>
</div>
