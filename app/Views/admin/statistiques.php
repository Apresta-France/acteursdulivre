...
<div class="admin-page stats-page">
    ...
    <?php foreach ($kpis as $k): ?>
      <div class="admin-kpi">
        ...
        <?php if (!empty($k['delta'])): ?>
          <div class="stats-delta is-<?= e((string) $k['delta']['tone']) ?>"><?= e((string) $k['delta']['text']) ?> vs période préc.</div>
        <?php endif; ?>
      </div>
    <?php endforeach; ?>
  </div>
  ...
  <div class="admin-card">
    ...
    <?php else: ?>
      ...
      <p class="admin-muted" style="margin-top: 10px;">
        Bleu : période choisie<?= $compare ? ' · Orange : période précédente' : '' ?>
        <?php if (empty($period['hourly'])): ?> · les visiteurs sont comptés une fois par jour<?php endif; ?>
      </p>
    <?php endif; ?>
  </div>
  ...
  <div class="stats-grid">
    <div class="admin-card">
      ...
      <?php $rankTable($paths ?? [], 'Aucune page vue sur cette période.'); ?>
    </div>
    <div class="admin-card">
      <h2>Prestataires les plus vus</h2>
      <?php $rankTable($profiles ?? [], 'Aucun profil consulté sur cette période.'); ?>
    </div>
    <div class="admin-card">
      <h2>Recherches</h2>
      <?php $rankTable($searches ?? [], 'Aucune recherche textuelle sur cette période.'); ?>
    </div>
    <div class="admin-card">
      <h2>Actions</h2>
      <?php $rankTable($actions ?? [], 'Aucune action enregistrée sur cette période.'); ?>
    </div>
    <div class="admin-card">
      <h2>Prestations les plus vues</h2>
      <?php $rankTable($services ?? [], 'Aucune fiche prestation vue.'); ?>
    </div>
    <div class="admin-card">
      <h2>Missions les plus vues</h2>
      <?php $rankTable($missions ?? [], 'Aucune fiche mission vue.'); ?>
    </div>
    <div class="admin-card">
      <h2>Articles du journal</h2>
      <?php $rankTable($articles ?? [], 'Aucun article consulté.'); ?>
    </div>
    <div class="admin-card">
      <h2>Pages métiers</h2>
      <?php $rankTable($metiers ?? [], 'Aucune page métier consultée.'); ?>
    </div>
    <div class="admin-card">
      <h2>Types de pages</h2>
      <?php $rankTable($pages ?? [], 'Aucun type de page pour le moment.'); ?>
    </div>
    <div class="admin-card">
      <h2>Recherches sans résultat</h2>
      <?php $rankTable($search_empty ?? [], 'Aucune recherche vide — ou pas encore de trafic.'); ?>
    </div>
    <div class="admin-card">
      <h2>Types de recherche</h2>
      <?php $rankTable($search_types ?? [], 'Aucun filtre de type.'); ?>
    </div>
    <div class="admin-card">
      <h2>Villes recherchées</h2>
      <?php $rankTable($search_cities ?? [], 'Aucune ville utilisée comme filtre.'); ?>
    </div>
    <div class="admin-card">
      <h2>Pages d’entrée</h2>
      <?php $rankTable($entries ?? [], 'Aucun premier passage enregistré.'); ?>
    </div>
    <div class="admin-card">
      <h2>Sources</h2>
      <?php $rankTable($referrers ?? [], 'Aucun référent externe.'); ?>
    </div>
    <div class="admin-card">
      <h2>Appareils</h2>
      <?php $rankTable($devices ?? [], 'Aucun appareil distingué.'); ?>
    </div>
  </div>
</div>
